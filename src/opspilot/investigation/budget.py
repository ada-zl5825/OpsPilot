...
from opspilot.investigation.evidence import query_fingerprint, tool_name_of
...
class ToolBudget(BaseModel):
    ...
    max_repeats_per_tool: int = Field(default=3, ge=1, le=16)
    max_repeats_per_query: int = Field(default=2, ge=1, le=8)
...
def evaluate_budget(
    ...
    *,
    ...
    if any(count > budget.max_repeats_per_tool for count in by_tool.values()):
