# OpsPilot 文档

完整设计原文：[`../OpsPilot_完整开发技术文档.md`](../OpsPilot_完整开发技术文档.md)

| 文档 | 内容 |
|---|---|
| [01_ARCHITECTURE.md](01_ARCHITECTURE.md) | 系统上下文、状态机、信任边界 |
| [02_DOMAIN_MODEL.md](02_DOMAIN_MODEL.md) | Incident / Evidence / Proposal 模型 |
| [03_MCP_TOOL_CONTRACTS.md](03_MCP_TOOL_CONTRACTS.md) | 工具目录与权限分类 |
| [04_SECURITY_MODEL.md](04_SECURITY_MODEL.md) | 写路径、审批、注入与脱敏 |
| [05_BENCHMARK.md](05_BENCHMARK.md) | 评测指标与门禁 |
| [06_OPERATIONS.md](06_OPERATIONS.md) | 本地启动与阶段命令 |
| [07_UPSTREAM_CONTRIBUTION.md](07_UPSTREAM_CONTRIBUTION.md) | HolmesGPT 上游 PR 路线 |
| [08_EXPERIMENT_REPORT.md](08_EXPERIMENT_REPORT.md) | Benchmark v1 离线实验报告 |
| [09_LIVE_AZURE_INVESTIGATION_FIX.md](09_LIVE_AZURE_INVESTIGATION_FIX.md) | Live Azure 调查失败：null 可选参数与预算重复计数修复 |
| [10_LIVE_EMPTY_EVIDENCE_FIX.md](10_LIVE_EMPTY_EVIDENCE_FIX.md) | Live 查空了：path 过滤、end 取整、rate 二次聚合与 Loki 查询修复 |
| [11_DIAGNOSIS_SCORING.md](11_DIAGNOSIS_SCORING.md) | 诊断打分层：slug 假 0、结构化 rubric、2026 RCA 评测对照 |
| [12_LIVE_CROSS_SCENARIO_RESIDUE.md](12_LIVE_CROSS_SCENARIO_RESIDUE.md) | Live 连跑残留：本场 onset 裁窗，S02–S04 不再吃上场 DB 日志 |
| [13_LIVE_QUIET_BEFORE_INJECT.md](13_LIVE_QUIET_BEFORE_INJECT.md) | Live 连跑：reset 后先等 5xx/error 静默再 inject；裁窗余量 30s→5s |
| [14_LIVE_TRACE_SUMMARY_PEERS.md](14_LIVE_TRACE_SUMMARY_PEERS.md) | Live S04：trace 摘要补 peer / 真实 error_count / 最慢 span |
| [HANDOFF.md](HANDOFF.md) | 新窗口交接：Phase 0–6 已完成；不要开始 UI / Multi-Agent / SFT |
| [UPSTREAM_BASELINE.md](UPSTREAM_BASELINE.md) | Phase 0 验收记录（已完成） |
